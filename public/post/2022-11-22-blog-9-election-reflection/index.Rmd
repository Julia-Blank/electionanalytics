...
As of 11/28, the official outcome of the 2022 election was that Democrat's won 48.14% of the two-party vote share and Republican's won 51.86% of the two-party vote share. [(reported by david leap)]() The graph of the actual vote share outcome versus my predicted vote share outcome is illustrated below. 
...
Overall, I think this model showed me a few key limitations of a national-level fundamentals model. In this section, I will break the major flaws into three larger themes: my fundamentally focused model created a static view of a more complex and dynamic electoral race, my national-level model oversimplified district level and state-level nuance which had a larger role in this 2022 election due to state level issues like abortion being on the ballot, and more technically my model's data sparsity limited the set of data and biased it towards training on a pattern of ------. 




**Hypothesis 1: The national level model failed to capture nuanced district, state, and regional nuance and variation that was especially dominant in this 2022 election** 
...
If I were to convert to a district level model (which I think is the best recourse), I would definitely try to solve the sample size issue using similar tactics to 538's CANTOR which takes data from similar districts and imputes it into districts with data sparcity. For much more advanced research, I would also look to seeing if there are applications where I could possibly generate [synthetic data through simulation](https://bipartisanpolicy.org/blog/synthetic-data/) which has seen positive effects in the past. 
...
https://www.sciencedirect.com/science/article/abs/pii/S0261379416304899
https://www.politico.com/newsletters/politico-nightly/2022/09/13/the-gops-last-candidate-quality-test-00056533
https://www.reuters.com/world/us/inflation-abortion-top-issues-us-voters-casting-midterm-ballots-exit-poll-finds-2022-11-08/
...
Regional Level Issue/national level model: 
Hollistically, we can look at abortion's salience as a voting issue. To get a baseline of its importance to voters outside their candidate choice, we can look at voter registration after Dobbs. Then we can 


**Hypothesis 1: Fundamentals Focused Model may have only allowed for a birds-eye view**

In retrospect, looking at the predictors in my model illustrates a very overhead picture of the election. Many of the variables I've included like incumbency and democratic seats are one's that won't change the whole election. Other predictors, like national generic ballot and polling 

FUNDEMANETALS FOCUSED MODEL:
- didn't consider candidate quality
- error with my model is that it contained a rather static view of the electoral race. this allowed it 
- TEST THIS BY:
- average score 
...
While I was a 
- biggest focus would be on two themes: prioritizing more district level predictions and ammasing more data to train on. 
- bias variance tradeoff; why i need to increase data 


- too little data to train on and it learned patterns. 
- going through historical examples
- similar election profile to 2012
- perhaps training on year with larger generic ballot gap?
